...
const HRDashboard = ({ user, onLogout }) => {
  ...
  const loadRequests = () => {
    const leaveRequests = JSON.parse(localStorage.getItem('leaveRequests') || '[]').filter(r => r.hr === 'Pending' && r.siteIncharge === 'Approved');
    const cancelRequests = JSON.parse(localStorage.getItem('cancelRequests') || '[]').filter(r => r.hr === 'Pending' && r.siteIncharge === 'Approved');
  ...
  const handleApproval = (request, action) => {
    if (request.type === 'Leave') {
      ...
      if (requestIndex !== -1) {
        requests[requestIndex].hr = action;

        if (action === 'Approved') {
          requests[requestIndex].finalStatus = 'Pending at SBU Head';
        } else {
          requests[requestIndex].finalStatus = 'Rejected by HR';
        }
      ...
      if (requestIndex !== -1) {
        cancelRequests[requestIndex].hr = action;

        if (action === 'Approved') {
          cancelRequests[requestIndex].finalStatus = 'Pending at SBU Head';
        } else {
          cancelRequests[requestIndex].finalStatus = 'Rejected by HR';
        }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          <div className="flex justify-between items-center flex-wrap gap-4">
            <div>
              <h1 className="text-3xl font-bold gradient-text mb-2">HR Dashboard</h1>
