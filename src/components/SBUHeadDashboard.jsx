...
import { LogOut, CheckCircle, XCircle, Download, Users, Clock, UserCheck, FileText, TrendingUp } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <ProgressChart
              title="SBU Head Approval Status"
              data={[
                { label: 'Approved', value: JSON.parse(localStorage.getItem('leaveRequests') || '[]').filter(r => r.sbuHead === 'Approved').length + JSON.parse(localStorage.getItem('cancelRequests') || '[]').filter(r => r.sbuHead === 'Approved').length, percentage: ((JSON.parse(localStorage.getItem('leaveRequests') || '[]').filter(r => r.sbuHead === 'Approved').length + JSON.parse(localStorage.getItem('cancelRequests') || '[]').filter(r => r.sbuHead === 'Approved').length) / (JSON.parse(localStorage.getItem('leaveRequests') || '[]').length + JSON.parse(localStorage.getItem('cancelRequests') || '[]').length) * 100).toFixed(1) || 0, color: '#22c55e' },
                { label: 'Rejected', value: JSON.parse(localStorage.getItem('leaveRequests') || '[]').filter(r => r.sbuHead === 'Rejected').length + JSON.parse(localStorage.getItem('cancelRequests') || '[]').filter(r => r.sbuHead === 'Rejected').length, percentage: ((JSON.parse(localStorage.getItem('leaveRequests') || '[]').filter(r => r.sbuHead === 'Rejected').length + JSON.parse(localStorage.getItem('cancelRequests') || '[]').filter(r => r.sbuHead === 'Rejected').length) / (JSON.parse(localStorage.getItem('leaveRequests') || '[]').length + JSON.parse(localStorage.getItem('cancelRequests') || '[]').length) * 100).toFixed(1) || 0, color: '#ef4444' },
